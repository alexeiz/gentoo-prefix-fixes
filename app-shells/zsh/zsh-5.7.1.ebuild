...
if [[ ${PV} == 9999* ]] ; then
	...
	NEWP=zsh-5.7.1
	SRC_URI="https://www.zsh.org/pub/${NEWP}.tar.xz
		https://www.zsh.org/pub/old/${NEWP}.tar.xz
		mirror://sourceforge/${PN}/${NEWP}.tar.xz
		doc? (
			https://www.zsh.org/pub/${NEWP}-doc.tar.xz
			mirror://sourceforge/${PN}/${NEWP}-doc.tar.xz
		)"
fi
...
src_configure() {
	...
	perl -i -pE 's/HAVE_NIS/DISABLE_NIS/g' "${S}"/configure.ac
